...
;; Vendored from https://github.com/erickgnavar/flymake-ruff (GPL-3.0+) and
;; taught to run remotely, so that ruff checks buffers visiting a container
;; over Tramp.  Upstream drives ruff with `call-process-region', which has no
;; file-name handler and so always runs on the local machine; the region is
;; instead handed to `process-file' through a temp file on the checker's own
;; host.  File names passed to ruff are stripped of their Tramp prefix, since
;; a checker in a container is given paths in its own filesystem.
      ...
      (let* ((config (and (project-current)
             ...
             (args (if config
                       ...
                       (if (member "check" program-args)
                           (append `("check" "--config" ,(file-local-name config))
                                   (cdr program-args))
                         (append `("--config" ,(file-local-name config))
                                 program-args))
                     program-args))
             (args (if code-filename
                       (append args `("--stdin-filename"
                                      ,(file-local-name code-filename)))
                     args))
             ...
             (default-directory (if (project-current)
                                  ...
                                  default-directory)))
        ;; Ruff replaces this temporary buffer with its stdout, which is why
        ;; the check does not run in the code buffer itself.
        (let ((stdin (make-nearby-temp-file "flymake-ruff-")))
          (unwind-protect
              (progn
                (write-region (point-min) (point-max) stdin nil 'silent)
                (erase-buffer)
                (apply #'process-file command stdin t nil args))
            (delete-file stdin))))
      (goto-char (point-min))
