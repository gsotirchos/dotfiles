...
;; Vendored from https://github.com/karlotness/flymake-quickdef (GPL-3.0+)
;; and taught to run remotely, so that backends built on it work in buffers
;; visiting a container over Tramp.  Upstream is unmaintained; three changes:
;;
;;   * the scratch directory is made with `make-nearby-temp-file', so it is
;;     created on the host the checker will actually run on;
;;   * `make-process' is passed :file-handler, without which it always spawns
;;     locally regardless of `default-directory';
;;   * CRs are stripped before the output is scanned, since a remote method
;;     that allocates a tty (Tramp does for ssh's direct-async, among others)
;;     reports CRLF line endings that the patterns would otherwise trip on;
;;   * `fmqd-temp-file' is bound to the *local* name of that file, since a
;;     remote checker is handed a path in its own filesystem, not a Tramp one.
;;     `file-local-name' is the identity locally, so :proc-form is unaffected
;;     for local buffers.
...
downstream errors inside Flymake."
  ...
  (let* ((def-docstring (when (stringp docstring) docstring))
         ...
         (temp-file-symb (make-symbol "fmqd-temp-file-remote"))
    ...
    `(defun ,name (report-fn &rest _args)
       ...
       (let* ((fmqd-source (current-buffer))
              ...
              ,@(when (eq write-type 'file)
                  `((,temp-dir-symb (make-nearby-temp-file "flymake-" t))
                    (,temp-file-symb
                     (concat
                      (file-name-as-directory ,temp-dir-symb)
                      (file-name-nondirectory (or (buffer-file-name) (buffer-name)))))
                    (fmqd-temp-file (file-local-name ,temp-file-symb))))
                                         ...
                                         (with-current-buffer fmqd-source
                                           (save-restriction
                                             ...
                                             (with-current-buffer (process-buffer proc)
                                               ;; A remote tty reports CRLF; drop
                                               ;; the CRs so the patterns below
                                               ;; see what they would locally.
                                               (goto-char (point-min))
